...
def addR(request):
    if request.method == 'POST':
        ...
        if form.is_valid():
            ...
            return redirect('/login/home/addR')
        else:
            messages.warning(request,"Please input valid Data")
            return HttpResponse('<h1>Invalid Data</h1>')
...
def addST(request):
    if request.method == 'POST':
        ...
        if form.is_valid():
            ...
            a.sname = data['sname']
            a.save()
            messages.success(request,"Station are added successfully")
            return redirect('/login/home/addST')
        else:
            messages.warning(request,"Please input valid Data")
            return HttpResponse('<h1>Invalid Data</h1>')
...
def addT(request):
    if request.method == 'POST':
        ...
        if form.is_valid():
            ...
            a.rid=r1
            a.save()
            messages.success(request,"Train are added successfully")
            return redirect('/login/home/addT')
        else:
            messages.warning(request,"Please input valid Data")
            return HttpResponse('<h1>Invalid Data</h1>')
...
def addRT(request):
    if request.method == 'POST':
        ...
        if form.is_valid():
            ...
            a.atime=data['atime']
            a.save()
            messages.success(request,"Touching station are added successfully")
            return redirect('/login/home/addRT')
        else:
            messages.warning(request,"Please input valid Data")
...
def book(request):
    ...
    if request.method=='POST':
        ...
        pay.pnr = cp+1
        pay.save()
        messages.success(request,"Tickets are booked")
        return render(request,"features/final.html",{'tname':tname,'tno':tno,'date':dt,'src':src,'des':des,'cls':cls,'pnr':(cp+1),'nos':nos,'dt':dt,'amt':price})
...
def bookingstatus(request):
    ...
    cont1 = Reservation.objects.all()
    cont2 = Trains.objects.all()
    return render(request,'features/viewreservation.html',{'cont1':cont1,'cont2':cont2})   
